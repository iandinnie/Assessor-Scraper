---
title: "Get Owner, Sales, & Building Info"
output: html_document
---

```{r}
library(tidyverse)
library(rvest)
library(RSelenium)
library(lubridate)
library(readxl)
library(rlist)
library(plyr)
library(hablar)
```

```{r}
# read in list of addresses_to_bind
addresses <- read_xlsx("Matched SF addresses.xlsx")$Address
```

```{r}
# this chunk initiates a new selenium server, if you get an error, it's usually because the port is already in use, change the first argument of rs(Driver) to four random digits followed by an L
rD <- rsDriver(2673L, "firefox")
rmDR <- rD[["client"]]
```

```{r}
get_assessor_info <- possibly(function(address){
  # get the current time and pause after running for 45 minutes
  currenttime <- now()
  if(currenttime > endtime){
    # if time is up, flag this record for removal (it gets filtered out in below chunk)
    return("Drop this")
    break 
  } else {
    # if time isn't up, proceed to grab the table
    print(paste0("It is now ",
                 currenttime,
                 ", I will stop at ",
                 endtime))
    # take small breaks so we look like a human
    Sys.sleep(runif(1,5,25))
    # navigate to the first page
    rmDR$navigate("http://gis.vgsi.com/nantucketma/Search.aspx")
    Sys.sleep(runif(1,1,5))
    
    # identify the search bar (try using the selector gadget chrome extension if unsure)
    search_bar <- rmDR$findElement("css selector", "#MainContent_txtSearchAddress")
    Sys.sleep(runif(1,1,5))
    
    # submit a query for the address
    search_bar$sendKeysToElement(list(address, key = "enter"))
    
    # take a quick breather while the content is sent over
    Sys.sleep(runif(1,1,5))
    
    # now look for it  
    address_link <- rmDR$findElement("link text", address)
    Sys.sleep(runif(1,1,3))
    
    # click the hyperlink taking you to the page for that specific property
    address_link$clickElement()
    Sys.sleep(runif(1,1,3))
    
    # copy down this url
    url <- rmDR$getCurrentUrl()
    Sys.sleep(runif(1,1,3))
    
################################################################################    
    # using rvest to pull in the info from the page
    
    # for the ith address, create a new item in the list containing the url for the ith address's assessor page
    url_pull <- url[[1]] %>%
    
    # read in the html  
    read_html()
    
    # initialize results list
    result_list <- list()
    
    categories <- c("Owner Name",
                    "Owner Address", 
                    "Attributes",
                    "Sales History")
    
    selectors <- c("#MainContent_lblOwner",
                   "#MainContent_lblAddr1",
                   "table#MainContent_ctl01_grdCns.GridViewStyle",
                   "table#MainContent_grdSales.GridViewStyle")
    
    # pull in the text attributes
    for(i in 1:2){
      result_list[[i]] <- url_pull %>% 
        html_nodes(css = selectors[i]) %>% 
        html_text()
    }
    # pull in the tables
    for(i in 3:4){
      table <- url_pull %>% 
        html_nodes(css = selectors[i]) %>% 
        html_table() %>% 
        tibble()
      if(i == 3){ # if pulling in attributes, convert to wide format and add address so we can merge
        table <- table[[1]][[1]] %>% 
          tibble() %>% 
          pivot_wider(names_from = colnames(table[[1]][[1]][1]),
                      values_from = colnames(table[[1]][[1]][2]))%>%
          mutate(Address = address)
      } else { # if pulling in sales history, just add the address
        table <- table[[1]][[1]] %>% 
          tibble() %>% 
          mutate(Address = address)
      }
      result_list[[i]] <- table
      }
    names(result_list) <- categories
    print(paste0("I successfully pulled ",
                 address,
                 "!"))
    return(result_list)
  }
},
    otherwise = "Address not Found")

```

```{r}
endtime <- now()+3*60*60

allinfo <- map(addresses_to_bind,get_assessor_info)

names(allinfo) <- addresses_to_bind[1:length(allinfo)]
allinfo <- allinfo[allinfo != "Drop this"]
save(allinfo, file = "allinfo3_29_pass1.RData")
```



```{r}
# make the data frames
# make a df of the owner information for all properties
owner_list <- list()
for(i in 1:length(allinfo_p3)){
  owner_list[[i]] <- list()
  owner_list[[i]][1] <- allinfo_p3[[names(allinfo_p3)[i]]][1]
  owner_list[[i]][2] <- allinfo_p3[[names(allinfo_p3)[i]]][2]
  owner_list[[i]] <- Reduce("rbind", owner_list[[i]]) %>%
    tibble() %>% 
    mutate(v = c("Owner Name", "Owner Address"))
  colnames(owner_list[[i]]) <- c("Value", "Variable")
  owner_list[[i]] <- owner_list[[i]] %>% 
  pivot_wider(names_from = Variable, values_from = Value) %>% 
  mutate(`Property Address` = names(allinfo_p3)[i])
}
Owners_df <- Reduce("rbind.fill", owner_list)

attribute_list <- list()
for(i in 1:length(allinfo_p3)){
  attribute_list[i] <- allinfo_p3[[names(allinfo_p3)[i]]][3]
}

Attribute_df <- Reduce("rbind.fill", attribute_list)


sales_list <- list()
for(i in 1:length(allinfo_p3)){
  sales_list[i] <- allinfo_p3[[names(allinfo_p3)[i]]][4]
}

Sales_df <- Reduce("rbind.fill", sales_list)
```

```{r}
# write files to csvs
Attribute_df <- Attribute_df %>% 
  tibble() %>% 
  convert(chr(names(Attribute_df)))
 
write_csv(Attribute_df,"Attributes_p1_3_30.csv")
write_csv(Owners_df,"Owner_p1_3_30.csv")
write_csv(Sales_df,"Sales_p1_3_30.csv")

# update the address list
#new_address_list <- addresses[match(names(allinfo_p2[length(allinfo_p2)]), addresses) + 1 :length(addresses)]
#new_address_list <- new_address_list[!is.na(new_address_list)]
```

```{r}
# run a second time with new address list

endtime <- now()+8*60*60

allinfo_p3 <- map(new_address_list,
            get_assessor_info)

names(allinfo_p3) <- new_address_list[1:length(allinfo_p3)]
allinfo_p3 <- allinfo_p3[allinfo_p3 != "Drop this"]
save(allinfo_p3, file = "allinfo3_30_pass1.RData")
```

```
```{r}

# this is a pretty dumb way of dealing with the fact that the tables pulled from the records sometimes have different dimensions
trouble_v <- c("16D MILESTONE RD","157 MAIN ST","130B MAIN ST","13 NEW ST","10G CROWS NEST WY","1 GULLY RD")
trouble_df <- allinfo2[trouble_v]
acquired <- names(allinfo2)[!names(allinfo2) %in% trouble_v]
allinfoproper <- allinfo2[acquired]

tobind <- list()
for(i in 1:length(names(allinfoproper))){
  try(allinfoproper[[names(allinfoproper)[i]]][["Attributes"]] <-
        
        allinfoproper[[names(allinfoproper)[i]]][["Attributes"]]%>%
    
        select(Address, `Style:`, Model, `Total Bedrooms:`, `Num Park`))
  
  if(allinfoproper[[names(allinfoproper)[i]]][["Attributes"]] %>% ncol() == 5){
  
    tobind[[length(tobind)+1]] <- allinfoproper[[names(allinfoproper)[i]]]
  
    names(tobind)[length(tobind)] <- tobind[[length(tobind)]][["Attributes"]]$Address
  }
}

addresses_to_bind <- names(tobind)
```
